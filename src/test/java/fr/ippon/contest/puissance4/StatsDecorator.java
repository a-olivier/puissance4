...
public class StatsDecorator {
	...
	public final synchronized <R> R collectSupplierStats(String supplierName,
			Supplier<R> supplier) {
	...
	public final synchronized <T, U, R> R collectBiFunctionStats(
			String supplierName, BiFunction<T, U, R> biFunction, T arg1, U arg2) {
		long top = System.nanoTime();
	...
	public final synchronized <T> void collectConsumerStats(
			String supplierName, Consumer<T> consumer, T arg) {
		long top = System.nanoTime();
		consumer.accept(arg);
		collectStats(supplierName, top);
	}

	public final synchronized void collectMethodStats(String supplierName,
			Method method) {
		long top = System.nanoTime();
		method.process();
		collectStats(supplierName, top);
	...
	public final synchronized <T, U> void collectBiConsumerStats(
			String supplierName, BiConsumer<T, U> consumer, T arg1, U arg2) {
		long top = System.nanoTime();
			...
			synchronized (statsExec) {
				for (String key : statsExec.keySet()) {
					...
					LOG.debug(key
							+ " : "
							+ statsMethode.size()
							+ " appels, de durée moyenne "
							+ Math.round(statsMethode.stream()
